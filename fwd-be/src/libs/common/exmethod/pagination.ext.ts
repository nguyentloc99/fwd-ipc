...
import {PaginateParams, Pagination, PaginateCursorParams} from '../models';
 ...
 * @param {(err: any, data: Pagination<Document[]>) => void} callback
...
mongoose.Query.prototype.paginate =
  function (params: PaginateParams = {}, callback?: (err: any, data: Pagination<Document[]>)
    => void): Promise<Pagination<Document[]>> {
    params = !params ? {} : params;
    return new Promise<Pagination<Document[]>>(
      (resolve, reject) => {
        if (!params) params = {} as PaginateParams;
        ...
        queryDocs.find(omitBy(merge(_where, this._conditions), isNil));
 ...
 */
mongoose.Query.prototype.paginateCursor =
  function (params: PaginateCursorParams = {}, callback?: (err: any, data: Pagination<Document[]>)
    => void): Promise<Pagination<Document[]>> {
    params = !params ? {} : params;
    return new Promise<Pagination<Document[]>>(
      (resolve, reject) => {
        ...
        async.parallel({
          ...
          if (!err && data) {
            ...
            if (result.data.length > 0) {
              ...
              result.hashPrevious = !!params.next || (result.totalPage > result.page && !!params.previous);
